...
function getStoreEnhancer() {
	if (!storeEnhancer) {
		// 延迟加载以避免循环依赖
		const { enhanceGridStoreWithDomain } = require('../../../domain/StoreEnhancer.js');
	}
	return storeEnhancer;
}

export function setStoreEnhancer(enhancer) {
...
function createUserGrid() {
	...
	return {
		...
		set: (pos, value) => {
			...
			if (storeEnhancer) {
				try {
					storeEnhancer.interceptSet(pos, value);
					// 如果成功，域层会通过 subscribe 更新 userGrid
					return;
				} catch (error) {
					console.warn('Domain set failed:', error);
					// 降级到直接设置（用于初始化阶段或错误情况）
				}
			}
